...
public class QuestionFrag extends Fragment implements OnRefreshListener,
	...
	public View onCreateView(final LayoutInflater inflater,
		...
		xt = (XueTuApplication) getActivity().getApplication();
		//得到所有没有图片的集合
	...
	public class MyOnclickListener implements OnClickListener {
		...
		public void onClick(View v) {
			...
			case R.id.right_layout:
				if (stu_id > 0) {
					Intent intent = new Intent(getActivity(), Question_ask.class);
					getActivity().startActivity(intent);
				} else {
					Toast.makeText(getActivity(), "请先登录哟！", Toast.LENGTH_SHORT).show();
				}
				break;
			case R.id.tv_sub1:
				sub_id = 1;
	...
	public void InitData(final int tempnum, int temp) {
		...
		hutils.send(HttpMethod.POST, url, params,
				new RequestCallBack<String>() {
					...
					@Override
					public void onFailure(HttpException arg0, String arg1) {
						Toast.makeText(getActivity(), "网络不太好哦！", Toast.LENGTH_SHORT).show();
