...
// implements OnTouchListener
{
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		intent_learning = new Intent(getActivity(), LearingRecordActivity.class);
		...
		public void handleMessage(Message msg) {
			...
			case UPDATE:
				if (msg.what == UPDATE) {
					...
					ObjectAnimator.ofPropertyValuesHolder(btn_center, visible).setDuration(500).start();
			...
			public void onSuccess(ResponseInfo<String> arg0) {
				...
				if (studyplan != null) {
					...
					if (is_today_studyplan) // 如果为true 则表明今天有计划
					{
						...
						if (p > 3600) {
							new AlertDialog.Builder(getActivity()).setTitle("提示").setMessage("离学习计划还有1个小时以上,是否提前学习")
									.setNegativeButton("取消", new DialogInterface.OnClickListener() {
								...
								public void onClick(DialogInterface dialog, int which) {
									...
									intent.putExtra("start_and_end_time",
											new SimpleDateFormat("HH:mm").format(studyplan.getStartTime()) + "~"
						...
						{
							if (p >= 0 && p <= 3600) // if true 执行设定的学习时间 if
														// false 执行剩余的时间
							{
								flag = false;
								planflag = false;
							...
							} else // 这里的p肯定为负数
							{
								...
								intent.putExtra("tag", 2);
								intent.putExtra("start_and_end_time",
										new SimpleDateFormat("HH:mm:ss").format(studyplan.getStartTime()) + " ~ "
						...
						planflag = false;
						center_click_flag = false;
						new AlertDialog.Builder(getActivity()).setTitle("注意").setMessage("当天没有可执行的学习计划,是否添加")
